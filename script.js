...
async function exportFilteredRecordsToPDF() {
  ...
  if (!container || !tbody || !window.html2canvas || !window.jspdf) {
  ...
  try {
    ...
    for (let pageIndex = 0; pageIndex < chunks.length; pageIndex++) {
      ...
      // فريم بسيط عشان نضمن إن الجدول اترسم فعليًا قبل التصوير
    ...
    pdf.save(`بيانات-الطلاب-${fileDate}.pdf`);
  } catch (error) {
    console.error(error);
    alert('حدث خطأ أثناء تجهيز ملف PDF. تأكد من الاتصال بالإنترنت وحاول مرة أخرى.');
  } finally {
    exportBtn.disabled = false;
    exportBtn.textContent = '📄 تصدير البيانات المفلترة PDF';
  }
}
