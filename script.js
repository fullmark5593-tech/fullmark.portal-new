    ...
    dayRecords.innerHTML = `
      ...
      ${daySummary.records
        .map((record) => {
          ...
          const bookPrice = parseFloat(record.bookPrice) || 0;
          const bookRemainingPrice = parseFloat(record.bookRemainingPrice) || 0;
          return `<span><strong>${record.studentName}</strong> - كود: ${record.studentCode} - المدرس: ${record.teacherName || '-'} - سعر الحصة: ${formatCurrency(lessonPrice)} - المبلغ المتبقي: ${formatCurrency(remainingPrice)} - سعر الكتاب: ${formatCurrency(bookPrice)} - المتبقي من سعر الكتاب: ${formatCurrency(bookRemainingPrice)}</span>`;
        })
        .join('')}
    `;
...
function collectTeacherScheduleRows(sectionIndex, teacherName) {
  const table = getScheduleSectionTables()[sectionIndex]?.table;
  if (!table) return [];
...
function sendTeacherScheduleWhatsapp() {
  ...
  const rows = collectTeacherScheduleRows(sectionIndex, teacherName);
  if (!rows.length) {
    alert('لا يوجد جدول مسجل لهذا المدرس حاليًا. تأكد من ملء بياناته في الجدول أولاً.');
    return;
  }
