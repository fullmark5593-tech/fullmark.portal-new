...
function computeDuplicatePhoneGroups() {
  ...
  const groups = [];
  phoneMap.forEach((entries, normalizedPhone) => {
    const distinctRootIds = new Set(entries.map((entry) => entry.rootId));
    ...
    if (distinctRootIds.size > 1) {
      groups.push({
        phone: entries[0].rawPhone,
        normalizedPhone,
        entries,
        studentsCount: distinctRootIds.size,
      });
    }
  });
  ...
  const rows = group.entries
    .map((entry) => `
      <div class="phone-group-row">
        <span class="phone-group-role">${entry.role}</span>
