      ...
      <div style="display: flex; flex-direction: column; gap: 6px;">
        ${versions.map((version) => `
          <button type="button" class="version-view-button" data-version-id="${version.id}"
            style="text-align: right; padding: 8px 12px; border-radius: 10px; cursor: pointer; border: 1px solid ${version.id === record.id ? '#2563eb' : '#cbd5e1'}; background: ${version.id === record.id ? 'rgba(37, 99, 235, 0.12)' : '#ffffff'}; color: #111827;">
            📅 ${version.registrationDate || 'غير محدد'}${version.id === record.id ? '  •  النسخة المعروضة الآن' : ''}
            <br><span style="font-size: 12px; color: #475569;">الحضور: ${version.presence || 'غير محدد'} — المتبقي: ${version.remainingPrice ?? 0} — المدرس: ${version.teacherName || '-'}</span>
          </button>
        `).join('')}
      </div>
    </div>
...
form.addEventListener('submit', async (event) => {
  ...
  try {
    ...
    // بتاريخ اليوم الحالي، والسجل الأصلي يظل كما هو بدون أي تغيير.
    ...
    const data = {
      ...
      registrationDate: isEditingExisting ? getTodayDateString() : formData.get('registrationDate'),
    ...
    if (editingId) {
      resultBox.textContent = isEditingExisting
        ? `تم حفظ التعديل كتسجيل إضافي جديد بتاريخ اليوم (${data.registrationDate})، والسجل الأصلي محفوظ كما هو.`
