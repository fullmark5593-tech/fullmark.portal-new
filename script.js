...
function populateAccountTeacherMultiselect() {
  const uniqueTeachers = Array.from(
    new Set(registrations.map((record) => (record.teacherName || '').trim()).filter(Boolean))
  ).sort((a, b) => a.localeCompare(b, 'ar'));

  const existingValues = Array.from(accountTeacherMultiselect.options).map((option) => option.value);
  const sameSet = existingValues.length === uniqueTeachers.length
    && existingValues.every((value, index) => value === uniqueTeachers[index]);
  if (sameSet) return;

  const previouslySelected = new Set(
    Array.from(accountTeacherMultiselect.selectedOptions).map((option) => option.value)
  );
  ...
  Array.from(accountTeacherMultiselect.options).forEach((option) => {
...
function getFilteredAccountRegistrations() {
  ...
  const selectedTeachers = Array.from(accountTeacherMultiselect.selectedOptions).map((option) => option.value);
  ...
  if (selectedTeachers.length) {
    filteredRegistrations = filteredRegistrations.filter(record =>
      selectedTeachers.includes(record.teacherName)
    );
  }
          ...
          .map((record) => {
            ...
            return `
              <div class="daily-record-row">
                ...
                <span>الصف: ${record.classGrade || '-'} — المدرس: ${record.teacherName || '-'}</span>
...
function adjustRecordsSectionHeight() {
  if (!recordsSection) return;
  const rect = recordsSection.getBoundingClientRect();
  const available = window.innerHeight - rect.top - 16;
  recordsSection.style.maxHeight = Math.max(240, available) + 'px';
}

window.addEventListener('resize', adjustRecordsSectionHeight);
window.addEventListener('load', adjustRecordsSectionHeight);
...
accountResetButton.addEventListener('click', () => {
  ...
  Array.from(accountTeacherMultiselect.options).forEach((option) => { option.selected = false; });
