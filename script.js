...
function getRecordsInCleanupRange() {
  ...
  return registrations.filter((record) => {
    ...
    // بنستثني "الكارت الأساسي" (أول سجل اتعمل لكل طالب — اللي id بتاعه هو
    // نفسه rootId) من الحذف نهائيًا مهما كان تاريخه داخل الفترة، ونحذف بس
    // النسخ/التكرارات اللي جت بعده (تسجيلات الحضور/الغياب اليومية التالية).
    const rootId = resolveRecordRootId(record);
    if (record.id === rootId) return false;
    return true;
  });
}
...
function updateCleanupPreview() {
  ...
  if (!fromValue && !toValue) {
    cleanupPreview.textContent = 'حدد الفترة أعلاه لمعاينة عدد نسخ التكرار اللي هتتحذف (الكارت الأساسي لكل طالب مش بيتحذف أبدًا).';
  ...
  if (!matching.length) {
    ...
    cleanupPreview.innerHTML = `⚠️ سيتم حذف <strong>${matching.length}</strong> نسخة تكرار (والكارت الأساسي لكل طالب هيفضل موجود) في الفترة (${rangeLabel}).`;
...
cleanupDeleteBtn.addEventListener('click', async () => {
  ...
  const firstConfirm = confirm(
    `تحذير: هيتم حذف ${matching.length} نسخة تكرار نهائيًا في الفترة (${rangeLabel}).\n\nالكارت الأساسي لكل طالب (أول سجل اتعمل له) مش هيتحذف ومش هيتأثر خالص — الحذف هيشمل بس نسخ الحضور/الغياب/تجديد الاشتراك اللاحقة داخل الفترة دي.\n\nهل أنت متأكد؟`
  );
  if (!firstConfirm) return;

  const secondConfirm = confirm(
    `تأكيد أخير: هتحذف ${matching.length} نسخة تكرار نهائيًا (الكارت الأساسي هيفضل زي ما هو). اضغط "موافق" فقط لو متأكد 100%.`
  );
  ...
  try {
    ...
    cleanupPreview.textContent = `✅ تم حذف ${matching.length} نسخة تكرار بنجاح (الكارت الأساسي لكل طالب موجود زي ما هو).`;
