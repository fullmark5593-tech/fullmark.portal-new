...
const absentRepeatOnly = document.getElementById('absent-repeat-only');
...
function computeAbsentStudentsData() {
  ...
  familyMap.forEach((latestRecord, rootId) => {
    ...
    absentStudents.push({
      ...
      lastAbsenceDate: latestRecord.registrationDate || '',
...
function getFilteredAbsentStudents() {
  ...
  if (absentRepeatOnly.checked) {
    list = list.filter((student) => student.streak >= 2);
  }
...
function buildAbsentCardHtml(student) {
  ...
  return `
    <div class="absent-card${isRepeat ? ' repeat-alert' : ''}" data-root-id="${student.rootId}">
      <span class="absent-streak-badge${isRepeat ? ' repeat' : ''}">${isRepeat ? '🚨' : '⚠️'} ${student.streak} ${student.streak === 1 ? 'حصة غياب' : 'حصص متتالية'}</span>
...
absentRepeatOnly.addEventListener('change', renderAbsentStudentsPanel);
...
absentResetBtn.addEventListener('click', () => {
  ...
  absentDateTo.value = '';
  absentRepeatOnly.checked = false;
  renderAbsentStudentsPanel();
});
