    ...
    <div class="modal-row" style="flex-direction: column; align-items: stretch; margin-top: 6px;">
      ...
      <div style="display: flex; flex-direction: column; gap: 6px;">
        ${versions.map((version) => `
          <div class="version-item" style="display: flex; align-items: stretch; gap: 6px; border: 1px solid ${version.id === record.id ? '#2563eb' : '#cbd5e1'}; border-radius: 10px; overflow: hidden; background: ${version.id === record.id ? 'rgba(37, 99, 235, 0.12)' : '#ffffff'};">
            <button type="button" class="version-view-button" data-version-id="${version.id}"
              style="flex: 1; text-align: right; padding: 8px 12px; border: none; background: transparent; cursor: pointer; color: #111827;">
            ...
            <button type="button" class="version-delete-button" data-version-id="${version.id}" title="حذف هذه النسخة"
              style="padding: 8px 14px; border: none; border-right: 1px solid ${version.id === record.id ? '#2563eb' : '#cbd5e1'}; background: #fee2e2; color: #dc2626; cursor: pointer; font-weight: 700; font-size: 15px;">✕</button>
          </div>
        `).join('')}
      </div>
    </div>
